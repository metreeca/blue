...
describe("factories", () => {
	...
	describe("resource", () => {
		...
		it("rejects a non-resource shape as the constraints", async () => {

			expect(() => resource(string() as never, {})).toThrow(TypeError);
		...
		describe("predicates", () => {
			...
			it("resolves a member name against the stated space", async () => {
				...
				const shape = resource({ space: schema }, { name: required(string()) });
