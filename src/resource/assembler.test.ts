...
describe("flatten", () => {
	...
	describe("inheritance", () => {
		...
		it("rejects re-pointing an inherited link at an unrelated target deferred to break a cycle", async () => {
			...
			const Wider = resource({ pattern: "/wider/{id}" }, { id: id() });
			const Unrelated = resource({ pattern: "/unrelated/{id}" }, { id: id() });
	...
	describe("classes", () => {
		...
		const Thing = resource({ class: "https://schema.org/Thing" }, { id: id() });
		...
		it("carries the class of every shape extended", async () => {
			...
			const Place = resource({ class: "https://schema.org/Place" }, { id: id() });
