	...
	describe("validatePatch", () => {
		...
		describe("resource constraints", () => {
			...
			describe("closed shape", () => {
				...
				it("accepts empty patch on empty shape", async () => {

					const shape = resource({});

					expect(validatePatch([{}], shape)).toEqual([]);

				});

				it("accepts patch with only defined properties", async () => {
					...
					expect(validatePatch([{ name: "Alice", age: 30 }], shape)).toEqual([]);
				...
				it("accepts declared id property", async () => {
					...
					expect(validatePatch([{ "id": "app:/users/123", name: "Alice" }], shape)).toEqual([]);
				...
				it("accepts properties from inherited shape", async () => {
					...
					expect(validatePatch([{ id: 1, name: "Alice" }], Derived)).toEqual([]);
				...
				it("rejects unknown property on empty shape", async () => {
					...
					expect(validatePatch([{ name: "Alice" }], shape).length).toBeGreaterThan(0);
				...
				it("rejects unknown property", async () => {
					...
					expect(validatePatch([{ name: "Alice", extra: "value" }], shape).length).toBeGreaterThan(0);
				...
				it("rejects undeclared id property", async () => {
					...
					expect(validatePatch([{ "id": "app:/users/123", name: "Alice" }], shape).length).toBeGreaterThan(0);
			...
			describe("custom validators skipped", () => {
				...
				it("accepts failing custom validator", async () => {
					...
					expect(validatePatch([{ age: 15 }], shape)).toEqual([]);
				...
				it("accepts failing inherited validator", async () => {
					...
					expect(validatePatch([{ age: 15, name: "Bob" }], Derived)).toEqual([]);
			...
			describe("trace structure", () => {
				...
				it("returns string array for value-level violations", async () => {
					...
					const trace = validatePatch([{ age: -5 }], shape);
				...
				it("includes property path in nested traces", async () => {
					...
					const trace = validatePatch([{ address: { city: "" } }], shape);
					const dict = trace.find(e => typeof e === "object") as Record<string, unknown> | undefined;
				...
				it("includes both unknown and invalid property traces in dictionary", async () => {
					...
					const trace = validatePatch([{ address: { city: "" }, extra: "value" }], shape);
					const dict = trace.find(e => typeof e === "object") as Record<string, unknown> | undefined;
		...
		describe("id constraints", () => {
			...
			it("accepts single absolute IRI", async () => {
				...
				expect(validatePatch([{ "id": "app:/users/123" }], shape)).toEqual([]);
			...
			it("rejects missing id", async () => {
				...
				expect(validatePatch([{}], shape).length).toBeGreaterThan(0);
			...
			it("rejects non-IRI value", async () => {
				...
				expect(validatePatch([{ "id": "not an iri" }], shape).length).toBeGreaterThan(0);
			...
			it("rejects multiple values", async () => {
				...
				expect(validatePatch([{ "id": ["/users/1", "/users/2"] }], shape).length).toBeGreaterThan(0);
			...
			describe("pattern", () => {
				...
				it("rejects id not matching pattern", async () => {
					...
					expect(validatePatch([{ "id": "/products/123" }], shape).length).toBeGreaterThan(0);
			...
			describe("in", () => {
				...
				it("accepts id in allowed enumeration", async () => {
					...
					expect(validatePatch([{ "id": "app:/users/alice" }], shape)).toEqual([]);
				...
				it("rejects id not in allowed enumeration", async () => {
					...
					expect(validatePatch([{ "id": "app:/users/charlie" }], shape).length).toBeGreaterThan(0);
			...
			describe("hasValue", () => {
				...
				it("accepts id matching hasValue", async () => {

					const shape = resource({ hasValue: ["app:/users/admin"] }, { id: id() });

					expect(validatePatch([{ "id": "app:/users/admin" }], shape)).toEqual([]);

				});

				it("rejects id not matching hasValue", async () => {

					const shape = resource({ hasValue: ["app:/users/admin"] }, { id: id() });

					expect(validatePatch([{ "id": "app:/users/guest" }], shape).length).toBeGreaterThan(0);
		...
		describe("type constraints", () => {
			...
			it("accepts single absolute IRI", async () => {
				...
				expect(validatePatch([{ "type": "app:/types/Person" }], shape)).toEqual([]);
			...
			it("rejects non-IRI value", async () => {
				...
				expect(validatePatch([{ "type": "not an iri" }], shape).length).toBeGreaterThan(0);
			...
			it("rejects multiple values", async () => {
				...
				expect(validatePatch([{ "type": ["/types/A", "/types/B"] }], shape).length).toBeGreaterThan(0);
		...
		describe("property constraints", () => {
			...
			describe("deletion markers", () => {
				...
				it("accepts null on scalar property", async () => {

					const shape = resource({
						name: required(string())
					});

					expect(validatePatch([{ name: null }], shape)).toEqual([]);

				});

				it("accepts null on array property", async () => {

					const shape = resource({
						tags: repeatable(string())
					});

					expect(validatePatch([{ tags: null }], shape)).toEqual([]);
			...
			describe("cardinality enforced for present properties", () => {

				it("accepts present scalar value on required property", async () => {

					const shape = resource({
						name: required(string())
					});

					expect(validatePatch([{ name: "Alice" }], shape)).toEqual([]);

				});

				it("accepts present array value on repeatable property", async () => {

					const shape = resource({
						tags: repeatable(string())
					});

					expect(validatePatch([{ tags: ["a", "b"] }], shape)).toEqual([]);
				...
				it("rejects scalar value on array property", async () => {
					...
					expect(validatePatch([{ tags: "a" }], shape).length).toBeGreaterThan(0);
		...
		describe("value constraints", () => {
			...
			describe("boolean values", () => {
				...
				it("accepts valid boolean", async () => {
					...
					expect(validatePatch([{ active: true }], shape)).toEqual([]);
					expect(validatePatch([{ active: false }], shape)).toEqual([]);
				...
				it("rejects non-boolean value", async () => {
					...
					expect(validatePatch([{ active: "true" }], shape).length).toBeGreaterThan(0);
			...
			describe("number values", () => {
				...
				it("accepts valid number", async () => {
					...
					expect(validatePatch([{ age: 25 }], shape)).toEqual([]);
				...
				it("rejects non-number value", async () => {
					...
					expect(validatePatch([{ age: "forty-two" }], shape).length).toBeGreaterThan(0);
			...
			describe("string values", () => {
				...
				it("rejects non-string value", async () => {
					...
					expect(validatePatch([{ name: 42 }], shape).length).toBeGreaterThan(0);
				...
				it("validates array elements individually", async () => {
					...
					expect(validatePatch([{ tags: ["abc", "de"] }], shape)).toEqual([]);
					expect(validatePatch([{ tags: ["abc", "x"] }], shape).length).toBeGreaterThan(0);
			...
			describe("local values", () => {
				...
				it("accepts valid local value", async () => {
					...
					expect(validatePatch([{ label: { "en": "Hello" } }], shape)).toEqual([]);
				...
				it("rejects non-local value", async () => {
					...
					expect(validatePatch([{ label: 42 }], shape).length).toBeGreaterThan(0);
			...
			describe("locals values", () => {
				...
				it("accepts valid locals value", async () => {
					...
					expect(validatePatch([{ labels: { en: ["Hello"], it: ["Ciao"] } }], shape)).toEqual([]);
				...
				it("rejects non-locals value", async () => {
					...
					expect(validatePatch([{ labels: "Hello" }], shape).length).toBeGreaterThan(0);
			...
			describe("resource values", () => {
				...
				it("accepts valid nested resource", async () => {
					...
					expect(validatePatch([{ address: { city: "Rome" } }], Person)).toEqual([]);

				});

				it("accepts absent nested resource", async () => {
					...
					expect(validatePatch([{}], Person)).toEqual([]);
				...
				it("rejects invalid nested resource", async () => {
					...
					expect(validatePatch([{ address: { city: 42 } }], Person).length).toBeGreaterThan(0);

				});

				describe("nested resources use full resource validation", () => {

					it("rejects missing required property in nested resource", async () => {
						...
						// top-level patch accepts missing properties, but nested resources don't
						expect(validatePatch([{ address: { city: "Rome" } }], Person).length).toBeGreaterThan(0);

					});

					it("enforces custom validators in nested resource", async () => {

						const validator: Validator = () => ["validation failed"];
						...
						// top-level patch skips custom validators, but nested resources enforce them
						expect(validatePatch([{ address: { city: "Rome" } }], Person).length).toBeGreaterThan(0);
			...
			describe("union values", () => {
				...
				it("accepts scalar matching one variant type", async () => {

					expect(validatePatch([{ value: "hello" }], textOrCount)).toEqual([]);
					expect(validatePatch([{ value: 42 }], textOrCount)).toEqual([]);

				});

				it("accepts missing union property", async () => {

					expect(validatePatch([{}], textOrCount)).toEqual([]);

				});

				it("accepts null as deletion marker on union property", async () => {

					expect(validatePatch([{ value: null }], textOrCount)).toEqual([]);
				...
				it("accepts array matching variant type and cardinality", async () => {
					...
					// multiUnion is repeatable (1..*): 3 values satisfy both type and cardinality
					expect(validatePatch([{ value: [1, 2, 3] }], multiUnion)).toEqual([]);

				});

				it("rejects scalar matching no variant type", async () => {

					expect(validatePatch([{ value: true }], textOrCount).length).toBeGreaterThan(0);
				...
				it("rejects array for scalar union range", async () => {
					...
					// textOrCount has maxCount=1: array is rejected
					expect(validatePatch([{ value: ["a", "b"] }], textOrCount).length).toBeGreaterThan(0);
				...
				it("accepts inherited union property", async () => {
					...
					expect(validatePatch([{ value: "hello" }], Derived)).toEqual([]);
					expect(validatePatch([{ value: true }], Derived).length).toBeGreaterThan(0);

				});

			});

			it("enforces inherited property constraints", async () => {
				...
				expect(validatePatch([{ name: 42 }], Derived).length).toBeGreaterThan(0);

			});

			it("enforces multiple parents property constraints", async () => {
				...
				expect(validatePatch([{ name: "Alice", age: 30 }], Person)).toEqual([]);
				expect(validatePatch([{ name: 42 }], Person).length).toBeGreaterThan(0);
				expect(validatePatch([{ age: "old" }], Person).length).toBeGreaterThan(0);
